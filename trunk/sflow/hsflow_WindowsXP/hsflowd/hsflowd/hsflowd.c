...
{ 
	...
	readInterfaces(&sp);
	HSPReadConfig(&sp);
	if (sp.adaptorList->num_adaptors >= 1){
		/*in_addr temp_ip;
		SFLAdaptor *adp = sp.adaptorList->adaptors[0];
		HSPAdaptorNIO *niostat = (HSPAdaptorNIO *)adp->userData;		
		temp_ip.S_un.S_addr = (ULONG)niostat->ipAddr.address.ip_v4.addr;
		char* ip = inet_ntoa(temp_ip);
		sp.host_hid.hostname.len = strlen(ip);
		sp.host_hid.hostname.str = ip;*/
		in_addr temp_ip;
		SFLAddress temp = sp.agentIP;
		temp_ip.S_un.S_addr = (u_long)temp.address.ip_v4.addr;
		char *ip = inet_ntoa(temp_ip);
		sp.host_hid.hostname.len = strlen(ip);
		strcpy(sp.host_hid.hostname.str, ip);
	}
	initAgent(&sp);
